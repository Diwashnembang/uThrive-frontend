...
export default function EventsPage() {
    ...
    if (searchTerm) {
      filtered = filtered.filter(
        (event) =>
          ...
          event.serviceProviderName.toLowerCase().includes(searchTerm.toLowerCase()),
      )
    }
  ...
  const handleUnregister = async (eventId: string) => {
    ...
    try {
      const result = unregisterFromEvent(eventId, user.id)
          ...
          <div className="flex gap-2">
            {user ? (
              ...
              <>
                ...
                <Button onClick={() => router.push("/auth/user/register")}>Sign Up</Button>
              </>
            )}
          </div>
        </div>
                  ...
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <CardDescription>by {event.serviceProviderName}</CardDescription>
                      </div>
                      <div className="flex flex-col gap-1">
                        {isRegistered && <Badge variant="default">Registered</Badge>}
                        {isFull && <Badge variant="destructive">Full</Badge>}
                        {isPast && <Badge variant="secondary">Past Event</Badge>}
                        {isUpcoming && !isFull && <Badge variant="outline">Available</Badge>}
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="flex-1 flex flex-col">
                    ...
                    <div className="mt-auto">
                      {isPast ? (
                        ...
                        <Button className="w-full" onClick={() => router.push("/auth/user/login")}>
                          Login to Register
                        </Button>
                      ) : isRegistered ? (
                        ...
                        <Button className="w-full" onClick={() => handleRegister(event.id)} disabled={isProcessing}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                ...
                <div>
                  <div className="text-2xl font-bold text-purple-600">
                    {user ? registrations.filter((r) => r.userId === user.id).length : 0}
                  </div>
